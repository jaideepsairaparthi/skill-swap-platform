...
const SkillList = () => {
  ...
  const [selectedSkills, setSelectedSkills] = useState({}); // Track selected skill per user
  ...
  const handleSkillSwap = async (targetUserId, skillName) => {
    ...
    try {
      console.log('Requesting skill swap with target user:', targetUserId, 'for skill:', skillName);
      const response = await requestSkillSwap(targetUserId, skillName);
      if (response?.error) {
        console.error('Skill swap request failed:', response.error);
        alert(`Skill swap failed: ${response.error}`);
      } else {
  ...
  const handleSkillSelect = (userId, skill) => {
    setSelectedSkills((prev) => ({
      ...prev,
      [userId]: skill, // Set the selected skill for the specific user
    }));
  };
            ...
            <div className="space-y-2">
              <p className="text-gray-700">
                ...
                {user.skillsOffered?.length ? (
                  user.skillsOffered.map((skill, index) => (
                    <span
                      ...
                      className={`inline-block bg-blue-100 text-blue-800 text-sm px-2 py-1 rounded-full mr-2 mb-2 cursor-pointer ${
                        selectedSkills[user.firebaseUID] === skill ? 'bg-blue-500 text-white' : ''
                      }`}
                  ...
                  <span className="text-gray-500">No skills offered</span>
                )}
              </p>
              <p className="text-gray-700">
                ...
                {user.skillsWanted?.length ? (
                  ...
                  <span className="text-gray-500">No skills wanted</span>
                )}
              </p>
            </div>
            <button
